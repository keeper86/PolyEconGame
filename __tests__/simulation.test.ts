...
const WORKER_PATH = path.resolve(__dirname, "../simulation/worker.ts");
...
describe("Simulation Worker", () => {
  ...
  beforeEach(() => {
    worker = new Worker(WORKER_PATH, {
      ...
      workerData: { tickIntervalMs: 200 }, // faster ticks for tests
    });
  });
  ...
  it(
    ...
    async () => {
      ...
      await new Promise<void>((resolve) => {
        worker.on("message", (msg) => {
          if (msg.type === "tick" && msg.tick >= 1) resolve();
        });
      });

      const pong = await new Promise<{ type: string; tick: number }>(
        (resolve) => {
          worker.on("message", (msg) => {
  ...
  it(
    ...
    async () => {
      const tickMsg = await new Promise<{ type: string; tick: number; elapsedMs: number }>(
        (resolve) => {
          worker.on("message", (msg) => {
            if (msg.type === "tick") resolve(msg);
          });
        }
      );
...
describe("WorkerManager", () => {
  it("startWorker spawns exactly one worker and stopWorker terminates it", async () => {
    const { startWorker, stopWorker, onWorkerMessage } = await import(
      "../simulation/workerManager"
    );
