...
function makeGameState(): GameState {
    ...
    return { tick: 5, planets: [planet], agents: [agent] };
}
...
describe('toImmutableGameState', () => {
    it('converts a GameState to a GameStateRecord', () => {
        ...
        expect(record.planets.size).toBe(state.planets.length);
        expect(record.agents.size).toBe(state.agents.length);
    });
    ...
    it('indexes planets by their id', () => {
        ...
        const planetId = state.planets[0].id;
        expect(record.planets.has(planetId)).toBe(true);
        expect(record.planets.get(planetId)?.name).toBe(state.planets[0].name);
    });
    ...
    it('indexes agents by their id', () => {
        ...
        expect(record.agents.get(agentId)?.name).toBe(state.agents[0].name);
    });

    it('converts multiple planets correctly', () => {
        const p1 = makePlanet('alpha');
        const p2 = makePlanet('beta');
        const state: GameState = { tick: 3, planets: [p1, p2], agents: [] };
...
describe('fromImmutableGameState', () => {
    it('round-trips a GameState through toImmutable / fromImmutable', () => {
        ...
        expect(restored.planets).toHaveLength(original.planets.length);
        expect(restored.agents).toHaveLength(original.agents.length);
        expect(restored.planets[0].id).toBe(original.planets[0].id);
    ...
    it('returns planet data objects that are identical to originals', () => {
        ...
        expect(restored.planets[0]).toBe(original.planets[0]);
        expect(restored.agents[0]).toBe(original.agents[0]);
    });
});
