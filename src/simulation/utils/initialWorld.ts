...
interface SmallPlanetSpec {
    ...
    totalArable: number;
    totalWater: number;
    govFacilityScale: number;
    companies: ACCompanySpec[];
...
function buildSmallPlanet(spec: SmallPlanetSpec): { planet: Planet; agents: Agent[] } {
    ...
    const govArableClaimId = `${spec.id}-gov-arable`;
    const govWaterClaimId = `${spec.id}-gov-water`;
    const govClaims: string[] = [govArableClaimId, govWaterClaimId];
    const govTenancies: string[] = [govArableClaimId, govWaterClaimId];

    const govAgriFacility = makeAgriculturalProduction(spec.id, `${spec.id}-gov`, spec.govFacilityScale);
    const govWaterFacility = makeWaterExtraction(spec.id, `${spec.id}-gov`, spec.govFacilityScale);
    const govStorage = makeStorage({
        planetId: spec.id,
        id: `${spec.id}-gov-storage`,
        name: `${spec.name} Gov. Storage`,
    });

    arableClaims.push({
        id: govArableClaimId,
        type: arableLandResourceType,
        quantity: spec.govFacilityScale * 1000,
        regenerationRate: spec.govFacilityScale * 1000,
        maximumCapacity: spec.govFacilityScale * 1000,
        claimAgentId: `${spec.id}-government`,
        tenantAgentId: `${spec.id}-government`,
        tenantCostInCoins: 0,
    });
    ...
    for (const company of spec.companies) {
        const arableClaimId = `${spec.id}-arable-${company.id}`;
        const waterClaimId = `${spec.id}-water-${company.id}`;
        const agriScale = company.arableLand / 1000;
        const waterScale = company.waterSource / 1000;
        ...
        arableClaims.push({
            ...
            regenerationRate: company.arableLand,
            maximumCapacity: company.arableLand,
            claimAgentId: `${spec.id}-government`,
        ...
        waterClaims.push({
            ...
            regenerationRate: company.waterSource,
            maximumCapacity: company.waterSource,
            claimAgentId: `${spec.id}-government`,
            tenantAgentId: company.id,
        ...
        agents.push({
            ...
            automatePricing: true,
            assets: {
                [spec.id]: makeAgentPlanetAssets(
                    spec.id,
                    [
                        makeWaterExtraction(spec.id, company.id, waterScale),
                        makeAgriculturalProduction(spec.id, company.id, agriScale),
                    ],
                    makeStorage({ planetId: spec.id, id: `${company.id}-storage`, name: `${company.name} Storage` }),
                    [],
                    [arableClaimId, waterClaimId],
                ),
            },
    ...
    const totalArable = arableClaims.reduce((s, c) => s + c.quantity, 0);
    const remainArable = spec.totalArable - totalArable;
    if (remainArable > 0) {
        const id = `${spec.id}-arable-unclaimed`;
        arableClaims.push({
            id,
            type: arableLandResourceType,
            quantity: remainArable,
    ...
    const totalWater = waterClaims.reduce((s, c) => s + c.quantity, 0);
    const remainWater = spec.totalWater - totalWater;
    if (remainWater > 0) {
        const id = `${spec.id}-water-unclaimed`;
        waterClaims.push({
            id,
            type: waterSourceResourceType,
            quantity: remainWater,
    ...
    const government: Agent = {
        id: `${spec.id}-government`,
...
// ============================================================================
// Planet specs: Gune, Icedonia, Pandara, Paradies, Suerte
// ============================================================================

const smallPlanetSpecs: SmallPlanetSpec[] = [
    {
        id: 'gune',
        name: 'Gune',
        population: 500_000,
        ...
        environment: {
            naturalDisasters: { earthquakes: 2, floods: 5, storms: 8 },
            pollution: { air: 1, water: 1, soil: 0 },
            regenerationRates: {
                air: { constant: 0.08, percentage: 0.08 / TICKS_PER_YEAR },
                water: { constant: 0.04, percentage: 0.04 / TICKS_PER_YEAR },
                soil: { constant: 0.003, percentage: 0.003 / TICKS_PER_YEAR },
            },
        },
    },
    {
        ...
        environment: {
            naturalDisasters: { earthquakes: 0, floods: 10, storms: 20 },
            pollution: { air: 0, water: 0, soil: 0 },
            regenerationRates: {
                air: { constant: 0.15, percentage: 0.15 / TICKS_PER_YEAR },
                water: { constant: 0.2, percentage: 0.2 / TICKS_PER_YEAR },
                soil: { constant: 0.002, percentage: 0.002 / TICKS_PER_YEAR },
            },
        },
    },
    {
        ...
        companies: [
            ...
            { id: 'pandara-sunleaf', name: 'Sunleaf Agriculture', arableLand: 20000, waterSource: 20000, wealth: 6e7 },
            {
        ...
        environment: {
            naturalDisasters: { earthquakes: 5, floods: 15, storms: 10 },
            pollution: { air: 3, water: 2, soil: 1 },
            regenerationRates: {
                air: { constant: 0.5, percentage: 0.5 / TICKS_PER_YEAR },
                water: { constant: 0.3, percentage: 0.3 / TICKS_PER_YEAR },
                soil: { constant: 0.02, percentage: 0.02 / TICKS_PER_YEAR },
            },
        },
    },
    {
        ...
        environment: {
            naturalDisasters: { earthquakes: 1, floods: 3, storms: 4 },
            pollution: { air: 1, water: 0, soil: 0 },
            regenerationRates: {
                air: { constant: 0.12, percentage: 0.12 / TICKS_PER_YEAR },
                water: { constant: 0.1, percentage: 0.1 / TICKS_PER_YEAR },
                soil: { constant: 0.01, percentage: 0.01 / TICKS_PER_YEAR },
            },
        },
    },
    {
        ...
        govFacilityScale: 30,
        companies: [
            { id: 'suerte-lucky-harvest', name: 'Lucky Harvest', arableLand: 25000, waterSource: 25000, wealth: 7e7 },
            { id: 'suerte-fortune-farms', name: 'Fortune Farms', arableLand: 20000, waterSource: 20000, wealth: 5e7 },
            { id: 'suerte-oasis-ag', name: 'Oasis Agriculture', arableLand: 15000, waterSource: 15000, wealth: 3e7 },
        ],
        infrastructure: {
            primarySchools: 100,
            secondarySchools: 50,
        ...
        environment: {
            naturalDisasters: { earthquakes: 3, floods: 8, storms: 12 },
            pollution: { air: 2, water: 1, soil: 1 },
            regenerationRates: {
                air: { constant: 0.2, percentage: 0.2 / TICKS_PER_YEAR },
                water: { constant: 0.15, percentage: 0.15 / TICKS_PER_YEAR },
                soil: { constant: 0.01, percentage: 0.01 / TICKS_PER_YEAR },
            },
        },
    },
 ...
 *  - Gune, Icedonia, Pandara, Paradies, Suerte — small to mid-size planets
 *
 * Total agents: 28 (Earth) + 4 (AC) + 14 (new planets) = 46
 */
export function createInitialGameState(): GameState {
    const { planet: earth, agents: earthAgents } = buildEarth();
    const { planet: alphaCentauri, agents: acAgents } = buildAlphaCentauri();
    const newPlanets = smallPlanetSpecs.map(buildSmallPlanet);

    const allAgents = [...earthAgents, ...acAgents, ...newPlanets.flatMap((p) => p.agents)];
    ...
    return {
        ...
        planets: new Map([
            ...
            ...newPlanets.map(({ planet }) => [planet.id, planet] as [string, Planet]),
        ]),
