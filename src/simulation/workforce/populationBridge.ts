...
export function hireFromPopulation(
    ...
    for (let age = MIN_EMPLOYABLE_AGE; age < demography.length; age++) {
        ...
        if (actual > 0) {
            ...
            const merged = mergeWealthMoments(hiredAccum, { mean: hiredWealthMean, variance: hiredWealthVar }, actual, srcW);
    ...
    if (remainder > 0) {
        for (let age = MIN_EMPLOYABLE_AGE; age < demography.length; age++) {
            ...
            if (take > 0) {
                ...
                const srcW = wealthDemography[age][edu].unoccupied;
                const merged = mergeWealthMoments(hiredAccum, { mean: hiredWealthMean, variance: hiredWealthVar }, take, srcW);
                hiredWealthMean = merged.mean;
                hiredWealthVar = merged.variance;
                hiredAccum += take;
            }
