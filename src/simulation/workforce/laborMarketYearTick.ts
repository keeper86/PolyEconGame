                ...
                for (const edu of educationLevelKeys) {
                    ...
                    if (srcCount > 0 && dstCount > 0) {
                        ...
                        dst.wealthMoments[edu] = mergeWealthMoments(
                            srcCount,
                            src.wealthMoments[edu],
                            dstCount,
                            dst.wealthMoments[edu],
                        );
                    } else if (srcCount > 0) {
                        dst.ageMoments[edu] = {
                        ...
                        dst.wealthMoments[edu] = {
                            mean: src.wealthMoments[edu].mean,
                            variance: src.wealthMoments[edu].variance,
                        };
                    } else if (dstCount > 0) {
                        dst.ageMoments[edu] = {
                    ...
                    for (let m = 0; m < NOTICE_PERIOD_MONTHS; m++) {
                        ...
                        if (srcDep > 0) {
                            dst.departingWealth[edu][m] = mergeWealthMoments(
                                dstDep,
                                dst.departingWealth[edu][m],
                                srcDep,
                                src.departingWealth[edu][m],
                            );
                        }
                        ...
                        if (srcRet > 0) {
                            dst.retiringWealth[edu][m] = mergeWealthMoments(
                                dstRet,
                                dst.retiringWealth[edu][m],
                                srcRet,
                                src.retiringWealth[edu][m],
                            );
                        }
