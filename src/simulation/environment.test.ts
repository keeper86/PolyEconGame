...
describe('environmentTick', () => {
    it('reduces pollution by constant and percentage and not below zero', () => {
        ...
        // air: 50 - 5 - 50 * 0.1 = 40
        expect(state.planets[0].environment.pollution.air).toBeCloseTo(40);

        // water: 10 - 2 - 10 * 0.5 = 3
        expect(state.planets[0].environment.pollution.water).toBeCloseTo(3);

        // soil: 2 - 1 - 2 * 0.5 = 0 -> clamp to 0
        expect(state.planets[0].environment.pollution.soil).toBe(0);
    });
    ...
    it('regenerates resources up to maximumCapacity and does not exceed it', () => {
        ...
        const entry = state.planets[0].resources.iron[0];
        ...
        environmentTick(state);
        expect(state.planets[0].resources.iron[0].quantity).toBe(60);
    });
});
