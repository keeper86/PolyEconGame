...
import { totalActiveForEdu } from '../workforce/workforceHelpers';
...
function hireWorkers(planet: Planet, agent: Agent, edu: keyof typeof agent.assets.p.allocatedWorkers, count: number): void {
...
describe('preProductionFinancialTick', () => {
    ...
    it('does nothing when agent has no workers', () => {
        ...
        expect(agent.deposits ?? 0).toBe(0);
    ...
    it('does not create a loan when firm already has enough deposits', () => {
        ...
        agent.deposits = 100; // more than enough

        preProductionFinancialTick(gs);

        expect(planet.bank!.loans).toBe(0);
        expect(agent.deposits).toBe(100 - 50); // 50 paid in wages
    });
...
describe('postProductionFinancialTick', () => {
    ...
    it('reduces household wealth by consumption for employed workers', () => {
        hireWorkers(planet, agent, 'none', 100);
        agent.deposits = 200;
        ...
        // Now run post-production: consumption should reduce wealth
        ...
        // Reset bank for post-production
        postProductionFinancialTick(gs);
        ...
        // Workers spent their wage on consumption
        expect(totalWealthAfter).toBeLessThan(totalWealthBefore);
    });
    ...
    it('distributes revenue to firms proportional to their workers', () => {
        ...
        const depositsBefore = agent.deposits ?? 0;
        postProductionFinancialTick(gs);

        // Agent should have received some revenue (>=0 after loan repayment)
        expect(agent.deposits).toBeGreaterThanOrEqual(0);
    });
