 ...
 *   avgBufferRatio = totalFoodStock     / (population * FOOD_TARGET_PER_PERSON)
 */
export type AggRow = {
    age: number;
    total: number;
...
export const FOOD_TARGET_PER_PERSON = FOOD_BUFFER_TARGET_TICKS * FOOD_PER_PERSON_PER_TICK;

/** Index into groupValues tuple: population count. */
export const GV_POP = 0 as const;
/** Index into groupValues tuple: total food stock (sum over all people in group). */
