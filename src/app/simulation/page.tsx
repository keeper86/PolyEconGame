     ...
     Each firm repays up to its full deposit balance:
         ...
         bank.deposits  −= repayment  (money supply ↓)`}
                ...
                <section id='pricing'>
                    ...
                    <p>
                        After production, each food-producing agent sets its offer price and quantity for the
                        upcoming market clearing step. The pricing algorithm uses a simple inventory-based
                        heuristic:
                    </p>
                    <pre className='bg-muted p-4 rounded-md text-sm overflow-x-auto'>
...
where
  ...
  adjustment     raises price when inventory is low, lowers when high`}
                    ...
                    <p className='mt-2'>
                        Agents that produce no food skip this step. The resulting per-agent offers are consumed
                        by the food market clearing step.
                    </p>
                </section>
                ...
                <section id='food-market'>
                    ...
                    <p>
                        The food market matches household demand with agent supply using a merit-order
                        dispatch mechanism. Agents are sorted by offer price (cheapest first), and
                        households purchase food until demand is satisfied or supply is exhausted.
                    </p>
                    <pre className='bg-muted p-4 rounded-md text-sm overflow-x-auto'>
...
foodPrice = volume-weighted average of clearing prices`}
                    ...
                    <p className='mt-2'>
                        Each household cell maintains a <code>foodStock</code> buffer. Food that is not consumed
                        in the current tick persists in the buffer. The volume-weighted average price is
                        recorded as the planet-level <code>foodPrice</code> for use by other subsystems.
                    </p>
                </section>
                ...
                <section id='transfers'>
                    ...
                    <p>
                        Family support flows redistribute wealth between age groups. Working-age adults
                        transfer a fraction of their income to dependent children and elderly household
                        members:
                    </p>
                    <pre className='bg-muted p-4 rounded-md text-sm overflow-x-auto'>
                        {`For each planet:
...
Global invariant: Σ all transfers = 0 (zero-sum)`}
                    ...
                    <p className='mt-2'>
                        The transfer matrix (<code>lastTransferMatrix</code>) is stored on the planet for
                        observability and frontend visualization.
                    </p>
                </section>
                ...
                <section id='wealth-diffusion'>
                    ...
                    <p>
                        A low-temperature variance smoothing step that gradually reduces extreme wealth
                        inequality within each age × education × occupation cell. This models informal
                        wealth-sharing mechanisms and prevents numerical divergence of wealth moments:
                    </p>
                    <pre className='bg-muted p-4 rounded-md text-sm overflow-x-auto'>
...
where diffusionRate is a small constant per tick`}
                    ...
                    <p className='mt-2'>
                        Wealth diffusion preserves the mean wealth of each cell while reducing variance.
                        This ensures that the wealth distribution remains well-behaved over long simulation
                        runs without affecting aggregate household deposit totals.
                    </p>
                </section>
  ...
  Age moments are updated to reflect the remaining (younger) workers.`}
                ...
                <section id='tick-order'>
                    ...
                    <p className='mt-4'>
                        ...
                        deterministically in workforce counts before the next tick begins. When{' '}
                        <code>SIM_DEBUG=1</code> is set, a single end-of-tick invariant check validates
                        population-workforce consistency and age-moment accuracy.
                    </p>
                </section>
                    ...
                    <table className='w-full text-sm border-collapse mt-4'>
                        ...
                        <tbody>
                            ...
                            <tr className='border-b'>
                                ...
                                <td className='p-2'>Centralized test fixtures, WorldBuilder for composable test setup</td>
                            </tr>
                            <tr className='border-b'>
                                <td className='p-2'>
                                    <code>debug/</code>
